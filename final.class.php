...

        {
                if ($queryType != "detail" && $queryType !="news") {
                        ...
                        try {
                                ...
                                $retData["status"]=0;
                                $retData["message"]="insert of '$stockTicker' for queryType: '$queryType' and jsonData: '$jsonData' accepted";
                        }
                        catch  (Exception $e) {
                                $retData["status"]=1;
                                $retData["message"]=$e->getMessage();
...
public static function getStock ($date)

        {
                if (empty($date)) {
                        $retData["status"]=1;
                        $retData["message"]="'$date' is empty";
                }
                else {
                        try {
                                $retData["result"] = GET_SQL("select * from stock where dateTime like ? order by dateTime DESC",$date."%");
                                $retData["status"]=0;
                        }
                        catch  (Exception $e) {
                                $retData["status"]=1;
